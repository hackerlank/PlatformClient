...
BOOL CGameFrameControl::OnInitDialog()
{
	...
	m_ChatMessage.SetBackgroundColor(false,RGB(21,6,0));
	m_ChatMessage.SetExpressionManager(CExpressionManager::GetInstance(),RGB(21,6,0));
	...
	case IDM_COPY_USER_NICKNAME:	//拷贝昵称
		{
			...
			{
				pEditChat->SetFocus();
				CWHService::SetClipboardString(pEditChat->GetText());
			}
...
VOID CGameFrameControl::OnBeginPaintWindow(HDC hDC)
{
	...
	rcChatArea.top=USER_VIEW_CY+(rcClient.Height()-USER_VIEW_CY)*INCISE_SCALE;
	...
	rcUserArea.bottom=USER_VIEW_CY+(rcClient.Height()-USER_VIEW_CY)*INCISE_SCALE-PROPERTY_CY;
...
VOID CGameFrameControl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	rcChatArea.top=USER_VIEW_CY+(nHeight-USER_VIEW_CY)*INCISE_SCALE;
	...
	rcListArea.bottom=USER_VIEW_CY+(nHeight-USER_VIEW_CY)*INCISE_SCALE-PROPERTY_CY;
	...
	if(pControlUI) pControlUI->SetPos(rcChatArea.right-34,rcChatArea.bottom-26);

	//短语按钮
	pControlUI=GetControlByName(szButtonChatShortControlName);
	if(pControlUI) pControlUI->SetPos(rcChatArea.right-58,rcChatArea.bottom-25);

	//聊天输入框
	pControlUI=GetControlByName(szEditChatControlName);
	if(pControlUI)
	{
		pControlUI->SetFixedHeight(18);
		pControlUI->SetFixedWidth(rcChatArea.Width()-22-58);		
		pControlUI->SetPos(rcChatArea.left+22,rcChatArea.bottom-24);
	}
