...
VOID CGameUpdateFrame::OnDrawClientArea(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	if(m_pDownLoadManager!=NULL && m_pDownLoadManager->m_pDownLoadSink!=NULL)
	{
		...
		//绘制点
		ImageProgressPoint.DrawImage(pDC,nProgressLeft+dwTotalProgress-ProgressPointSize.cx/2,37);
		ImageProgressPoint.DrawImage(pDC,nProgressLeft+dwCurrProgress-ProgressPointSize.cx/2,77);	
...
VOID CGameUpdateFrame::RectifyWindow(INT nWidth, INT nHeight)
{
	...
	{
		//设置按钮		
		m_btClose.SetWindowPos(NULL,379,7,0,0,SWP_NOZORDER|SWP_NOSIZE);
