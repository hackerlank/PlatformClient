...
//设置验证
bool CWndValidateCode::SetValidateCode(LPCTSTR pszValidateCode, UINT nMaxCount)
{
	ASSERT(pszValidateCode!=NULL);
	if (pszValidateCode==NULL) return false;

	lstrcpyn(m_szValidateCode,pszValidateCode,nMaxCount);
...
VOID CWndValidateCode::CreateValidateCode(CImage & ImageValidateCode, INT nWidth, INT nHeight)
{
	...
	//填充背景
	pBufferDC->FillSolidRect(1,1,nWidth-2,nHeight-2,RGB(136,126,93));
	pBufferDC->Draw3dRect(0,0,nWidth,nHeight,RGB(136,126,93),RGB(136,126,93));
...
VOID CWndValidateCode::OnPaint()
{
	...
	{
		//默认背景
		dc.FillSolidRect(&rcClient,RGB(136,126,93));
	}
