...
bool CMissionLogon::OnEventMissionRead(TCP_Command Command, VOID * pData, WORD wDataSize)
{
	if (Command.wMainCmdID==MDM_GP_LOGON)
	{
		...
		case SUB_GP_MATCH_SIGNUPINFO: //报名信息	
			{
				return OnSocketSubMatchSignupInfo(pData,wDataSize);
			}
		case SUB_GP_GROWLEVEL_CONFIG: //等级配置
			{
				return OnSocketSubGrowLevelConfig(pData,wDataSize);
			}
		case SUB_GP_QUERY_VERIFY_CODE:
			{
				return OnSocketSubQueryVerifyCode(pData,wDataSize);
			}
		case SUB_GP_VERIFY_CODE_RESULT:
			{
				return OnSocketSubVerifyCodeResult(pData,wDataSize);
			}
		}
	}
...
VOID CMissionLogon::ShowLogon(BYTE cbShowFalg)
{
	...
	{
		m_pDlgLogon->ShowWindow(cbShowFalg);
...
VOID CMissionLogon::ShowRegister(BYTE cbShowFalg)
{
	...
	//显示窗口
	m_pDlgRegister->ShowWindow(cbShowFalg);
...
//报名列表
bool CMissionLogon::OnSocketSubMatchSignupInfo(VOID * pData, WORD wDataSize)
{
	//校验数据
	...
	//提取数据			
	tagSignupMatchInfo * pSignupMatchInfo = (tagSignupMatchInfo *)pData;
	WORD wSignupCount = wDataSize/sizeof(pSignupMatchInfo);
...
//获取验证码
bool CMissionLogon::OnSocketSubQueryVerifyCode(VOID * pData, WORD wDataSize)
{
	//效验参数
...
//验证结果
bool CMissionLogon::OnSocketSubVerifyCodeResult(VOID * pData, WORD wDataSize)
{
	//效验参数
	ASSERT(wDataSize<=sizeof(CMD_GP_VerifyCodeResult));
	if (wDataSize>sizeof(CMD_GP_VerifyCodeResult)) return false;

	//提取数据
	CMD_GP_VerifyCodeResult * pVerifyCodeResult = (CMD_GP_VerifyCodeResult *)pData;
	ASSERT(pVerifyCodeResult!=NULL);
	...
	{
		CInformation Infomation;
		Infomation.ShowMessageBox(pVerifyCodeResult->szDescString);
	}
