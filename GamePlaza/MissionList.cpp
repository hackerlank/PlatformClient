...
bool CMissionList::OnEventMissionLink(INT nErrorCode)
{
	...
	{
		//状态效验
		if ((m_KindIDMission.GetCount()==0)&&(m_bUpdateOnLine==false)&&(m_bUpdateMatch==false))
		{
			ASSERT(FALSE);
			return false;
		}
	...
	if(m_bUpdateMatch==true)
	{
		...
		//发送数据
		CMissionManager * pMissionManager=GetMissionManager();
		pMissionManager->SendData(MDM_GP_SERVER_LIST,SUB_GP_GET_MATCH,NULL,0);
	}
...
bool CMissionList::OnEventMissionShut(BYTE cbShutReason)
{
	...
	//继续任务
	if ((m_KindIDMission.GetCount()>0)||(m_bUpdateOnLine==true) || (m_bUpdateMatch==true))
	{
		//创建窗口
		if (m_hWnd==NULL)
		{
...
//更新比赛
VOID CMissionList::UpdateMatchInfo()
{
	//状态判断
	ASSERT(m_bUpdateMatch==false);
	if(m_bUpdateMatch==true) return;
