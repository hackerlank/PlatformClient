...
void CMatchViewFrame::OnBeginPaintWindow(HDC hDC)
{
	...
	if(m_pGameMatch!=NULL)
	{
		...
		if(m_pGameMatch->cbMatchType==MATCH_TYPE_LOCKTIME)
		{
			...
			rcMatchInfo.OffsetRect(nXStartPos,nYStartPos);
			_sntprintf(szMatchFee,CountArray(szMatchFee),SCORE_STRING TEXT("%s"),m_pGameMatch->lMatchFee,m_pGameMatch->cbMatchFeeType==MATCH_FEE_TYPE_GOLD?TEXT("游戏币"):TEXT("元宝"));
			DrawText(hDC,szMatchFee,lstrlen(szMatchFee),rcMatchInfo,dwFormat);	

			//参数条件
			rcMatchInfo=m_MatchPosition.rcMatchCondition;
			rcMatchInfo.OffsetRect(nXStartPos,nYStartPos);
			LPCTSTR pszMatchCondition[]={TEXT("所有玩家"),TEXT("蓝钻会员"),TEXT("黄钻会员"),TEXT("白钻会员"),TEXT("红钻会员"),TEXT("VIP会员"),TEXT("晋级人员")};						
			//报名条件		
			...
			rcMatchInfo.OffsetRect(nXStartPos,nYStartPos);
			_sntprintf(szMinPlayCount,CountArray(szMinPlayCount),TEXT("%d 局"),m_pGameMatch->wMatchPlayCount);
			...
			if(m_ImageNumber1.IsNull()==false)
			{
				...
				//状态判断
				if(m_cbMatchStatus==MS_NULL)			//比赛未开始
				{
					//变量定义
		...
		if(m_pGameMatch->cbMatchType==MATCH_TYPE_IMMEDIATE)
		{
			...
			rcMatchInfo.OffsetRect(nXStartPos,nYStartPos);
			_sntprintf(szStartUserCount,CountArray(szStartUserCount),TEXT("满%d人开赛"),m_pGameMatch->wStartUserCount);
			...
			rcMatchInfo.OffsetRect(nXStartPos,nYStartPos);
			_sntprintf(szPlayCount,CountArray(szPlayCount),TEXT("%d 局"),m_pGameMatch->wMatchPlayCount);
...
bool CMatchViewFrame::OnEventViewData(WORD wCmdID, VOID * pData, WORD wDataSize)
{
	...
	case SUB_GR_USER_MATCH_STATUS:	//玩家参赛状态
		{
